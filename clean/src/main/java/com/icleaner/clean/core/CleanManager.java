package com.icleaner.clean.core;
...
import com.icleaner.clean.zcallback.AppCacheCallBack;
import com.icleaner.clean.zcallback.AppRamCallBack;
import com.icleaner.clean.zcallback.FileInfoCallBack;
import com.icleaner.clean.zcallback.SystemCacheCallBack;
import com.icleaner.clean.zcallback.UninstallResidualCallback;
import com.icleaner.clean.entity.JunkInfo;
import com.icleaner.clean.entity.Sizesort;
import com.icleaner.clean.filemanager.FileSortHelper;
...
public class CleanManager {
    ...
    public void startNotificationCleanService() {
        mContext.startService(new Intent(mContext, MyServiceNotificationMonitor.class));
    }

    public void closeNotificationCleanService() {
        mContext.stopService(new Intent(mContext, MyServiceNotificationMonitor.class));
    }
    ...
    public void loadUninstallResidual(UninstallResidualCallback uninstallResidualCallback) {
        String data = MyUtils.readFileFromAssets(mContext, "/raw/");
        uninstallSize = 0;
        ...
        if (!TextUtils.isEmpty(data)) {
            try {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    long size = MyUtils.getFileSize(file);
        ...
        if (listInfo != null) {
            for (AndroidAppProcess info : listInfo) {
                ...
                try {
                    ...
                    if (!MyUtils.isThirdApp(otherInfo) || packageName.equals(mContext.getPackageName()) || packageName.contains("com.icleaner")) {
                        continue;
                    }
                    if (ignoreApp.contains(packageName)) {
                        ...
                        appRam.isSelfBoot = MyUtils.isStartSelf(mContext.getPackageManager(), packageName);
    ...
    public void loadApkFile(FileInfoCallBack fileInfoCallBack) {
        PhoneFileCategoryHelper fileCategoryHelper = new PhoneFileCategoryHelper(mContext);
        Cursor cursor = fileCategoryHelper.query(PhoneFileCategoryHelper.FileCategory.Apk, FileSortHelper.SortMethod.size);
        apkSize = 0;
        apkFiles.clear();
        if (cursor != null && cursor.getCount() != 0) {
            ...
            do {
                long size = 0;
                try {
                    size = Long.parseLong(cursor.getString(PhoneFileCategoryHelper.COLUMN_SIZE));
                } catch (Exception e) {
                    continue;
                }
                String path = cursor.getString(PhoneFileCategoryHelper.COLUMN_PATH);
                String name = com.icleaner.clean.filemanager.Util.getNameFromFilepath(path);
                long date = cursor.getLong(PhoneFileCategoryHelper.COLUMN_DATE);
                apkSize += size;
    ...
    public void loadLogFile(FileInfoCallBack fileInfoCallBack) {
        PhoneFileCategoryHelper fileCategoryHelper = new PhoneFileCategoryHelper(mContext);
        Cursor cursor = fileCategoryHelper.query(PhoneFileCategoryHelper.FileCategory.Log, FileSortHelper.SortMethod.size);
        logSize = 0;
        logFiles.clear();
        if (cursor != null && cursor.getCount() != 0) {
            cursor.moveToFirst();
    ...
    public void loadSystemCache(final SystemCacheCallBack systemCacheCallBack) {
        ...
        for (int i = 0; i < packages.size(); i++) {
            ...
            if (!MyUtils.isThirdApp(packageInfo.applicationInfo) || TextUtils.equals(packageInfo.packageName, mContext.getPackageName())) {
...
//            mContext.startActivity(new Intent(Settings.ACTION_USAGE_ACCESS_SETTINGS).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
    ...
    public Method getPackageSizeInfoMethod(Context context) {
        ...
        try {
            ...
            MyUtils.log(TAG, "loadSystemCache error mGetPackageSizeInfoMethod=null");
            e.printStackTrace();
        }
    ...
    public void removeAppCache(JunkInfo appCache) {
        MyUtils.deleteFile(appCache.path);
    ...
    public void removeFilesOfUnintalledApk(JunkInfo uninstallResidual) {
        MyUtils.deleteFile(uninstallResidual.path);
    ...
    public void removeApkFiles(JunkInfo fileInfo) {
        MyUtils.deleteFile(fileInfo.path);
