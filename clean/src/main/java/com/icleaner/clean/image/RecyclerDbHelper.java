package com.icleaner.clean.image;
...
//        db.endTransaction();
    ...
    private String getRecyclerDirectory(String recyclerTime) {
        return Environment.getExternalStorageDirectory() + "/icleanerbackup/" + recyclerTime + "/";
    }
    ...
    private void deleteOverDateRecyclerFile() {
        File file = new File(Environment.getExternalStorageDirectory() + "/icleanerbackup");
            ...
            for (int i = 0; i < files.length; i++) {
                ...
                try {
                    ...
                    if (System.currentTimeMillis() > date.getTime() + RECYCLER_AUTO_DELETE_INTERVAL) {
                        UtilsFile.deleteFile(mContext, files[i].getAbsolutePath());
                    }
                } catch (Exception e) {
                    continue;
                }
    ...
    public boolean putImageToRecycler(ImageInfo imageInfo) {
        ...
        if (!file.exists() || file.isDirectory()) {
            MyUtils.log("rqy", "putImageToRecycler: file not exist or is directory, " + imageInfo.path);
            return isSuccess;
        }
        ...
        if (rowId < 0) {
            MyUtils.log("rqy", "putImageToRecycler: rowId= " + rowId);
            return isSuccess;
        }
        imageInfo.rowId = rowId;
        String destDic = RecyclerDbHelper.getInstance(mContext).getRecyclerDirectory(recyclerTime);
        String result = UtilsFile.copyFileToRecycler(imageInfo.path, destDic, "img_" + rowId);
        if (result == null) {
            MyUtils.log("rqy", "copyFileToRecycler:error");
            int deleteRowId = RecyclerDbHelper.getInstance(mContext).deleteItem(imageInfo);
            MyUtils.log("rqy", "deleteRowId--" + deleteRowId);
            return isSuccess;
        }
        boolean deleteSuc = UtilsFile.deleteFile(mContext, imageInfo.path);
        if (deleteSuc) {
    ...
    public boolean restoreImageFromRecycler(ImageInfo imageInfo) {
        ...
        String result = UtilsFile.copyFileToRecycler(imageInfo.backFilePath, directory, name);

        if (result != null) {
            new MediaScanner(mContext).scanFile(imageInfo.restoreFilePath, "image/jpeg");
            int success = deleteItem(imageInfo);
            UtilsFile.deleteFile(mContext, imageInfo.backFilePath);
