package com.vector.mcleaner.manager;
...
import com.vector.mcleaner.mCallback.AppCacheCallBack;
import com.vector.mcleaner.mCallback.AppRamCallBack;
import com.vector.mcleaner.mCallback.FileInfoCallBack;
import com.vector.mcleaner.mCallback.SystemCacheCallBack;
import com.vector.mcleaner.mCallback.UninstallResidualCallback;
import com.vector.mcleaner.entity.JunkInfo;
import com.vector.mcleaner.entity.Sizesort;
...
public class CleanManager {
    ...
    public void startNotificationCleanService() {
        mContext.startService(new Intent(mContext, MyNotificationMonitorService.class));
    }

    public void closeNotificationCleanService() {
        mContext.stopService(new Intent(mContext, MyNotificationMonitorService.class));
    }

    public void startLoad() {
        Intent intent = new Intent(mContext, MyCleanService.class);
        ...
        if (listInfo != null) {
            for (AndroidAppProcess info : listInfo) {
                ...
                try {
                    ...
                    if (!com.vector.mcleaner.mutil.Util.isThirdApp(otherInfo) || packageName.equals(mContext.getPackageName()) || packageName.contains("com.vectorapps")) {
                        continue;
                    }
                    if (ignoreApp.contains(packageName)) {
                        ...
                        appRam.isSelfBoot = com.vector.mcleaner.mutil.Util.isStartSelf(mContext.getPackageManager(), packageName);
    ...
    public void loadUninstallResidual(UninstallResidualCallback uninstallResidualCallback) {
        String data = com.vector.mcleaner.mutil.Util.readFileFromAssets(mContext, "/raw/");
        uninstallSize = 0;
        ...
        if (!TextUtils.isEmpty(data)) {
            try {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    long size = com.vector.mcleaner.mutil.Util.getFileSize(file);
    ...
    public void loadApkFile(FileInfoCallBack fileInfoCallBack) {
        ...
        if (cursor != null && cursor.getCount() != 0) {
            ...
            do {
                ...
                String name = FileUtil.getNameFromFilepath(path);
    ...
    public void loadSystemCache(final SystemCacheCallBack systemCacheCallBack) {
        ...
        for (int i = 0; i < packages.size(); i++) {
            ...
            if (!com.vector.mcleaner.mutil.Util.isThirdApp(packageInfo.applicationInfo) || TextUtils.equals(packageInfo.packageName, mContext.getPackageName())) {
...
//            mContext.startActivity(new Intent(Settings.ACTION_USAGE_ACCESS_SETTINGS).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
    ...
    public Method getPackageSizeInfoMethod(Context context) {
        ...
        try {
            ...
            com.vector.mcleaner.mutil.Util.log(TAG, "loadSystemCache error mGetPackageSizeInfoMethod=null");
            e.printStackTrace();
        }
    ...
    public void removeAppCache(JunkInfo appCache) {
        com.vector.mcleaner.mutil.Util.deleteFile(appCache.path);
    ...
    public void removeFilesOfUnintalledApk(JunkInfo uninstallResidual) {
        com.vector.mcleaner.mutil.Util.deleteFile(uninstallResidual.path);
    ...
    public void removeApkFiles(JunkInfo fileInfo) {
        com.vector.mcleaner.mutil.Util.deleteFile(fileInfo.path);
