...
import com.cleaner.callback.AppCacheCallBack;
import com.cleaner.callback.AppRamCallBack;
import com.cleaner.callback.FileInfoCallBack;
import com.cleaner.callback.SystemCacheCallBack;
import com.cleaner.callback.UninstallResidualCallback;
import com.cleaner.sqldb.CleanDBHelper;
import com.cleaner.entity.JunkInfo;
import com.cleaner.entity.Sizesort;
...
public class CleanManager {
    ...
    public void startNotificationCleanService() {
        mContext.startService(new Intent(mContext, NotificationsMonitorService.class));
    }

    public void closeNotificationCleanService() {
        mContext.stopService(new Intent(mContext, NotificationsMonitorService.class));
    }
        ...
        if (listInfo != null) {
            for (AndroidAppProcess info : listInfo) {
                ...
                try {
                    ...
                    if (!Util.isThirdApp(otherInfo) || packageName.equals(mContext.getPackageName()) || packageName.contains("com.brudermobi")) {
                        continue;
                    }
    ...
    public void loadApkFile(FileInfoCallBack fileInfoCallBack) {
        ...
        Cursor cursor = fileCategoryHelper.query(FileCategoryHelper.FileCategory.Apk, SortHelperFile.SortMethod.size);
        apkSize = 0;
        apkFiles.clear();
        if (cursor != null && cursor.getCount() != 0) {
            ...
            do {
                ...
                String name = com.cleaner.filemanager.Util.getNameFromFilepath(path);
    ...
    public void loadLogFile(FileInfoCallBack fileInfoCallBack) {
        FileCategoryHelper fileCategoryHelper = new FileCategoryHelper(mContext);
        Cursor cursor = fileCategoryHelper.query(FileCategoryHelper.FileCategory.Log, SortHelperFile.SortMethod.size);
        logSize = 0;
        logFiles.clear();
        if (cursor != null && cursor.getCount() != 0) {
            cursor.moveToFirst();
