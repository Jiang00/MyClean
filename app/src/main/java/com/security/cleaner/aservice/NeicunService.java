...
import com.security.cleaner.presenter.FloatMemoryManager;
import com.security.cleaner.presenter.GetTopApp;
...
public class NeicunService extends Service {
...
//        manager.addWindowsView();
    ...
    private void updata() {
        manager.upDate(Util.getMemory(NeicunService.this));
        new Thread(new Runnable() {
            @Override
            public void run() {
                String pkg = topApp.execute();
//                if (hmoes.contains(pkg)) {
//                    manager.upDate(FileUtil.getMemory(NeicunService.this));
//                    manager.addWindowsView();
//                } else {
//                    manager.removeWindowsView();
//                }
                if (!TextUtils.equals(pkg, runingGboost)) {
                    runingGboost = pkg;
                    if (PreData.getDB(NeicunService.this, Constant.TONGZHILAN_SWITCH, true)) {
                        ArrayList<String> gboost_names = CleanDBHelper.getInstance(NeicunService.this).getWhiteList(CleanDBHelper.TableType.GameBoost);
                        if (gboost_names.contains(pkg)) {
                            Intent intent = new Intent(NeicunService.this, NotificationService.class);
    ...
    public void onCreate() {
        super.onCreate();
        manager = FloatMemoryManager.getInstance(NeicunService.this);
        mana = (WindowManager) NeicunService.this.getSystemService(Context.WINDOW_SERVICE);
