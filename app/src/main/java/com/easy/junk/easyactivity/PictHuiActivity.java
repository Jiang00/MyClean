...
public class PictHuiActivity extends BaseActivity {
    ...
    class HuiAdapter extends RecyclerView.Adapter<HuiAdapter.HomeViewHolder> {
        ...
        public void restore(MyCallBack callback) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    for (ImageInfo info : list) {
                        if (!info.isNormal) {
                            RecyclerDbHelper.getInstance(PictHuiActivity.this).restoreImageFromRecycler(info);
                            rostore.add(info);
                            isRestore = true;
                            if (callBack != null) {
                                callBack.progress();
        ...
        public void delete(MyCallBack callback) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    for (ImageInfo info : list) {
                        if (!info.isNormal) {
                            RecyclerDbHelper.getInstance(PictHuiActivity.this).deleteImageFromRecyler(info);
                            delete.add(info);
                            if (callBack != null) {
                                callBack.progress();
                            }
        ...
        public HuiAdapter.HomeViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            HuiAdapter.HomeViewHolder holder = new HuiAdapter.HomeViewHolder(LayoutInflater.from(
                    PictHuiActivity.this).inflate(R.layout.layout_hui_item, parent,
                    false));
            return holder;
        }
        ...
        public void onBindViewHolder(final HuiAdapter.HomeViewHolder holder, final int position) {
            ...
            if (cachebitmap != null) {
                ...
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        final Bitmap bitma = imageHelper.pathWithScaledBitmap(PictHuiActivity.this, info.backFilePath, MyUtils.dp2px(112), MyUtils.dp2px(112));
                        if (bitma == null) {
                        } else {
