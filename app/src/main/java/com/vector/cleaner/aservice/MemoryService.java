...
import com.vector.cleaner.presenter.FloatMemoryManager;
import com.vector.cleaner.presenter.GetTopApp;
...
//        manager.addWindowsView();
    ...
    private void updata() {
        manager.upDate(Util.getMemory(MemoryService.this));
        new Thread(new Runnable() {
            @Override
            public void run() {
                String pkg = topApp.execute();
//                if (hmoes.contains(pkg)) {
//                    manager.upDate(FileUtil.getMemory(MemoryService.this));
//                    manager.addWindowsView();
//                } else {
...
//                }
                if (!TextUtils.equals(pkg, runingGboost)) {
                    runingGboost = pkg;
                    if (PreData.getDB(MemoryService.this, Constant.TONGZHILAN_SWITCH, true)) {
