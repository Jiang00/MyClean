...
public final class PreData {
    ...
    private static SharedPreferences getDB(Context context) {
        if (db == null && context != null)
            db = context.getSharedPreferences(Contents.SHARED_FILE, 0);
        return db;
    }
    ...
    public static void addName(Context context, String name) {
        String nameList = getDB(context, Contents.WHILT_LIST, "");
        if (nameList != null) {
            ...
            if (!lst.contains(name)) {
                ...
                putDB(context, Contents.WHILT_LIST, nameList);
            }
        } else {
            putDB(context, Contents.WHILT_LIST, name);
        }
    }
