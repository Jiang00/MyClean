...
public final class ShortcutSuperUtils {
    ...
    public static void updateShortcutIcon(Context context, String title, Intent intent, Bitmap bitmap) {
        ...
        try {
            ...
            Log.e("intent_url", intent.toUri(0));
            int index = -1;
//            c.moveToFirst();
//            do {
//                Log.e("jfy", "title=" + c.getString(c.getColumnIndex("title")) + "intent=" + c.getString(c.getColumnIndex("intent")));
//            } while (c.moveToNext());
            ...
            if (c != null && c.getCount() > 0) {
                ...
                Log.i("HJ", "url=" + uri.toString() + "update ok: affected " + i + " rows,index is" + index);
            } else {
