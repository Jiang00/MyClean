...
public class SimilarPictureAdapter extends RecyclerView.Adapter<CountryViewHolder> {
    ...
    public SimilarPictureAdapter(Context context, int headerMode, ArrayList<ArrayList<ImageInfo>> similarItem) {
        mContext = context;
        ...
        for (int i = 0; i < similarItems.size(); i++) {
            ...
            for (ImageInfo lg : similarItems.get(i)) {
                LineItem lineItem = new LineItem(lg.path, false, sectionManager, headerCount + sectionFirstPosition);
                lineItem.otherInfo = lg.avgPixel + "\n" + lg.sourceHashCode;
                mItems.add(lineItem);
            }
            headerCount++;
...
//        lp.setSlm(item.sectionManager == LINEAR ? LinearSLM.ID : GridSLM.ID);
        ...
        lp.setColumnWidth(CommonUtil.dp2px(96));
