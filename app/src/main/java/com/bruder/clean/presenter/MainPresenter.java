...
public class MainPresenter extends BasePresenter<MainMyView> {
    private int cpuTemp = 40;

    public MainPresenter(MainMyView iView, Context context) {
        super(iView, context);
        this.iView = iView;
    ...
    public void clickRotate(boolean isGood) {
        ...
        DataPre.putDB(context, Constant.IS_ROTATE, true);
        setRotateGone();
    }

    public void setRotateGone() {
        if (DataPre.getDB(context, Constant.IS_ROTATE, false)) {
            iView.setRotateGone();
        }
    }
