...
//        handler.sendEmptyMessage(NEED_INVALIDATE);//向handler发送一个消息，让它开启重绘
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int circleRadius = getResources().getDimensionPixelSize(R.dimen.d109);
        mPaintCircle.setColor(ContextCompat.getColor(context, R.color.B3));
        //依次旋转画布，画出每个刻度
        for (int i = 1; i <= 60; i++) {
            canvas.save();//保存当前画布
            canvas.rotate(360 / 60 * i, width / 2, getResources().getDimensionPixelSize(R.dimen.d138));
            //左起：起始位置x坐标，起始位置y坐标，终止位置x坐标，终止位置y坐标，画笔(一个Paint对象)
            canvas.drawLine(width / 2, getResources().getDimensionPixelSize(R.dimen.d138) - circleRadius, width / 2, getResources().getDimensionPixelSize(R.dimen.d138) - circleRadius + dip2px(context, 6), mPaintCircle);
            canvas.restore();//
        }
        // 设置颜色
