...
import com.privacy.junk.presenterprivacy.GetTopApp;
import com.privacy.junk.presenterprivacy.PrivacyFloatStateManager;
...
public class PrivacySuspensionBallService extends Service {
...
//        manager.addWindowsView();
    ...
    private void updata() {
        ...
        count--;
        manager.upDate(MyUtils.getMemory(PrivacySuspensionBallService.this));
...
//                }
                if (!TextUtils.equals(pkg, runingGboost)) {
                    runingGboost = pkg;
                    if (PreData.getDB(PrivacySuspensionBallService.this, MyConstantPrivacy.TONGZHILAN_SWITCH, true)) {
                        ArrayList<String> gboost_names = CleanDBHelper.getInstance(PrivacySuspensionBallService.this).getWhiteList(CleanDBHelper.TableType.GameBoost);
                        if (gboost_names.contains(pkg)) {
                            Intent intent = new Intent(PrivacySuspensionBallService.this, PrivacyNotificationService.class);
