...
import com.privacy.clean.cleanimage.ImageHelper;
import com.privacy.clean.cleanimage.ImageInfo;
...
import com.privacy.clean.cleanimage.RecyclerDbHelper;
...
public class PrivacyPictHuiActivity extends BaseActivity {
...
//                    dialog(adapter.checkNum());
                    ...
                    if (adapter.checkNum() > 0) {
                        dialog(true, adapter.checkNum(), getResources().getString(R.string.picture_6));
                    }
                    break;
                case R.id.picture_delete:
                    if (adapter.checkNum() > 0) {
                        dialog(false, adapter.checkNum(), getResources().getString(R.string.picture_7));
                    }
                    break;
            }
        }
    ...
    protected void findId() {
        ...
        pichui_button_lin = (LinearLayout) findViewById(R.id.pichui_button_lin);
    }

    private void addAd() {
        nativeView = SetAdUtilPrivacy.getNativeAdView(TAG_RECYCLE, R.layout.native_ad_3);
    ...
    private void dialog(final boolean isrestore, final int count, final String titleName) {
...
//        dialog_title.setText(titleName);
    ...
    class HuiAdapter extends RecyclerView.Adapter<HuiAdapter.HomeViewHolder> {
        ...
        public void restore(MyCallBack callback) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    for (ImageInfo info : list) {
                        if (!info.isNormal) {
                            RecyclerDbHelper.getInstance(PrivacyPictHuiActivity.this).restoreImageFromRecycler(info);
                            rostore.add(info);
                            isRestore = true;
                            if (callBack != null) {
                                callBack.progress();
        ...
        public void delete(MyCallBack callback) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    for (ImageInfo info : list) {
                        if (!info.isNormal) {
                            RecyclerDbHelper.getInstance(PrivacyPictHuiActivity.this).deleteImageFromRecyler(info);
                            delete.add(info);
                            if (callBack != null) {
                                callBack.progress();
                            }
        ...
        public HuiAdapter.HomeViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            HuiAdapter.HomeViewHolder holder = new HuiAdapter.HomeViewHolder(LayoutInflater.from(
                    PrivacyPictHuiActivity.this).inflate(R.layout.layout_hui_item, parent,
                    false));
            return holder;
        }
        ...
        public void onBindViewHolder(final HuiAdapter.HomeViewHolder holder, final int position) {
            ...
            if (cachebitmap != null) {
                ...
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        final Bitmap bitma = imageHelper.pathWithScaledBitmap(PrivacyPictHuiActivity.this, info.backFilePath, MyUtils.dp2px(112), MyUtils.dp2px(112));
                        if (bitma == null) {
                        } else {
            ...
            holder.picture_time3.setText((MyUtils.millTransFate(RecyclerDbHelper.RECYCLER_AUTO_DELETE_INTERVAL - (time - last)) + 1) + " " + getText(R.string.picture_11));
