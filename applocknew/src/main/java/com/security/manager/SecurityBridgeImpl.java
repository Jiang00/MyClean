    ...
    public boolean check(String passwd, boolean normal) {
        if (SecurityMyPref.checkPasswd(passwd, normal)) {
            if (context instanceof Activity) {
                ((SecurityPatternEosActivity) context).unlockSuccess(unlockMe);
            } else {
                if (context == null) {
                    try {
                        ((SecurityPatternEosActivity) App.getContext()).unlockSuccess(unlockMe);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                } else {
                    ((SecurityEosService) context).unlockSuccess(unlockMe);
        ...
        if (menus == null) {
            ...
            SecurityFlowMenu forget = new SecurityFlowMenu() {
                ...
                public void onClick(View v) {
                    ...
                    if (hasPermission) {
                        /*
                        View resetView = ResetPasswd.getView(context, true, ((SecurityEosService)context).handler, new ResetPasswd.IResetListener() {
                            @Override
                            public void onSuccess() {
                                ((SecurityEosService)context).hideAlertIfPossible(false);
...
//                    if (hasPermission) {
//                        View themeView = Themes.getView(context, ((SecurityEosService) context).handler, new Runnable() {
//                            @Override
//                            public void run() {
//                                ((SecurityEosService) context).hideAlertImmediate();
...
//        }
    ...
    public void toggle(boolean normal) {
        if (unlockSelf) {
            ((SecurityPatternEosActivity) App.getContext()).toggle(normal);
        }
    }
    ...
    public void back() {
        if (!hasPermission || context instanceof Activity) {
            ((SecurityPatternEosActivity) context).onBackPressed();
        } else {
            ((SecurityEosService) context).backHome();
        }
    }
