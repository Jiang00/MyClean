...
import com.eos.manager.meta.SecurityDaily;
import com.eos.manager.meta.SecurityFlowMenu;
...
import com.eos.manager.page.MessageBox;
    ...
    public boolean check(String passwd, boolean normal) {
        if (SecurityMyPref.checkPasswd(passwd, normal)) {
            if (context instanceof Activity) {
                ((AppLockPatternEosActivity) context).unlockSuccess(unlockMe);
            } else {
                if (context == null) {
                    try {
                    ...
                    ((AppLockEosService) context).unlockSuccess(unlockMe);
        ...
        if (menus == null) {
            ...
            SecurityFlowMenu forget = new SecurityFlowMenu() {
                ...
                public void onClick(View v) {
                    ...
                    if (hasPermission) {
                        /*
                        View resetView = ResetPasswd.getView(context, true, ((AppLockEosService)context).handler, new ResetPasswd.IResetListener() {
                            @Override
                            public void onSuccess() {
                                ((AppLockEosService)context).hideAlertIfPossible(false);
                        ...
                        */
            /*
                        AlertDialog dialog = AppLockResetPattern.showDialog(context, new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                context.startActi*vity(new Intent(context, AppLockResetPattern.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK).putExtra("direct-confirm", true));
                                ((AppLockEosService) context).hideAlertIfPossible(false);
                                detachFromWindow(MENU_IDX_FORGET);
                            }
                        }, null, hasPermission);
                        ...
                        context.startActivity(new Intent(context, AppLockResetPattern.class));
                    }
                }
            }.init();
*/
...
//                    if (hasPermission) {
//                        View themeView = Themes.getView(context, ((AppLockEosService) context).handler, new Runnable() {
//                            @Override
//                            public void run() {
//                                ((AppLockEosService) context).hideAlertImmediate();
...
//        }
    ...
    public void toggle(boolean normal) {
        if (unlockSelf) {
            ((AppLockPatternEosActivity) App.getContext()).toggle(normal);
        }
    }
    ...
    public void back() {
        if (!hasPermission || context instanceof Activity) {
            ((AppLockPatternEosActivity) context).onBackPressed();
        } else {
            ((AppLockEosService) context).backHome();
        }
    }
