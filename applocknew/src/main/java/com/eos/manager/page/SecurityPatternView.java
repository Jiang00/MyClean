...
public class SecurityPatternView extends View {
    ...
    */
    ...
    public SecurityPatternView(Context context) {
        this(context, null);
    }

    Drawable circle;
    Drawable circleSolid;
    Drawable circleRed;
    Drawable dotSolid;
    Drawable dot;
    boolean hidePath;
    ...
    public SecurityPatternView(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        try {
            hidePath = App.getSharedPreferences().getBoolean("hide_path", false);
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//        final int height = h - getPaddingTop() - getPaddingBottom();
    ...
    private Cell detectAndAddHit(float x, float y) {
        ...
        if (cell != null) {
            ...
            if (mEnableHapticFeedback && false) {
    ...
    */
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mPatternDisplayMode == DisplayMode.Animate) {
            ...
            // + 1 so we toggleProtectStatus on complete pattern
    ...
    private void drawCircle(Canvas canvas, int leftX, int topY, boolean partOfPattern) {
        ...
        if (!partOfPattern || (mInStealthMode && mPatternDisplayMode != DisplayMode.Wrong)) {
            ...
            outerCircle = circleSolid;
            innerCircle = dotSolid;
            if (hidePath) {
                sizeInner /= 2;
            }
//            size *= 2;
            ...
            outerCircle = circleRed;
            innerCircle = circleRed;
//            size *= 2;
...
//        final int height = mBitmapHeight;
        ...
        int offsetX = (int) ((squareWidth) / 2f);
        int offsetY = (int) ((squareHeight) / 2f);
...
//        mCircleMatrix.preTranslate(-mBitmapWidth/2, -mBitmapHeight/2);
        ...
        //canvas.setMatrix(mCircleMatrix);

        outerCircle.setBounds(leftX - size, topY - size, leftX + size, topY + size);
        outerCircle.draw(canvas);

        innerCircle.setBounds(leftX - sizeInner, topY - sizeInner, leftX + sizeInner, topY + sizeInner);
...
//        canvas.drawBitmap(innerCircle, mCircleMatrix, mPaint);
    ...
    private static class SavedState extends BaseSavedState {
        ...
        /**
         * Constructor called from {@link SecurityPatternView#onSaveInstanceState()}
         */
