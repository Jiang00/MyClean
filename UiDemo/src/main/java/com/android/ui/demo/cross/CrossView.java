...
//            crossView = new CrossView(mContext);
        ...
        protected String doInBackground(String... params) {
            ...
            headBitmap = Utils.getBitmap(object.optString("head"));
            iconBitmap = Utils.getBitmap(object.optString("icon"));
            return null;
        }
...
//        }
    ...
    private void setServiceData(JSONObject object, Builder builder) {
        ...
        if (subTitle != null) {
            String subTitleText = object.optString("subTitle");
        ...
        if (actionBtn != null) {
            ...
            if (Utils.checkoutISAppHasInstalled(mContext, pkgName)) {
                actionText = object.optString("actionApply");
            } else {
                actionText = object.optString("actionInstall");
            }
    ...
    private void setDefaultData(Builder builder) {
        ...
        if (head != null && builder.headImage > 0) {
            head.setImageResource(builder.headImage);
        }
        if (icon != null && builder.mainIconId > 0) {
            icon.setImageResource(builder.mainIconId);
        }
        if (title != null) {
            if (builder.titleTextId > 0) {
                title.setText(builder.titleTextId);
                try {
                    title.setTextColor(builder.titleColor);
                } catch (Exception e) {
                    e.printStackTrace();
        ...
        if (actionBtn != null) {
            ...
            if (builder.actionText > 0) {
                actionBtn.setText(builder.actionText);
                try {
                    actionBtn.setTextColor(builder.actionBtnTextColor);
                } catch (Exception e) {
                    e.printStackTrace();
