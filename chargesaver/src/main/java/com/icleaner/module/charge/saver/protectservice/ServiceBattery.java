package com.icleaner.module.charge.saver.protectservice;
...
import com.icleaner.module.charge.saver.BatteryProtectActivity;
...
import com.icleaner.module.charge.saver.Utils.WidgetContainer;
...
public class ServiceBattery extends Service {
    ...
    private void showChargeView() {
        boolean isChargeScreenSaver = (boolean) Utils.readData(this, BatteryConstants.CHARGE_SAVER_SWITCH, true);
        if (!isChargeScreenSaver) {
            return;
        }
        if ((Boolean) Utils.readData(this, BatteryConstants.IS_ACTIVITY, true)) {
            if (entry == null || !entry.isCharging()) {
                return;
            }
            Intent intent = new Intent().addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            if (Utils.readData(this, BatteryConstants.KEY_SAVER_TYPE, BatteryConstants.TYPE_HOR_BAR).equals(BatteryConstants.TYPE_HOR_BAR)) {
                intent.putExtra("type", "bar");
            } else {
                intent.putExtra("type", "duck");
            }
            intent.setClass(this, BatteryProtectActivity.class);
