...
public class IndicatorMyView extends LinearLayout implements ViewPager.OnPageChangeListener {
    /**
     * 需要创建的指示器个数
     */
    ...
    public IndicatorMyView(Context context) {
        this(context, null);
    }

    public IndicatorMyView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public IndicatorMyView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        // 获取自定义属性
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.IndicatorMyView);
        normalBp = drawableToBitamp(ta.getDrawable(R.styleable.IndicatorMyView_normalDrawable));
        selectBp = drawableToBitamp(ta.getDrawable(R.styleable.IndicatorMyView_selectDrawable));
        mInterval = ta.getDimensionPixelOffset(R.styleable.IndicatorMyView_indicatorInterval, 6);
        normalColor = ta.getColor(R.styleable.IndicatorMyView_normalColor, Color.GRAY);
        selectColor = ta.getColor(R.styleable.IndicatorMyView_selectColor, Color.RED);
        mRadius = ta.getInteger(R.styleable.IndicatorMyView_indicatorRadius, 6);
        ta.recycle();
        // 初始化
        init();
    }
