...
public class ADLabel extends View {
    ...
    public ADLabel(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.Charge_Label);
        mTextContent = ta.getString(R.styleable.Charge_Label_charge_text);
        mTextColor = ta.getColor(R.styleable.Charge_Label_charge_text_color, Color.parseColor("#ffffff"));
        mTextSize = ta.getDimension(R.styleable.Charge_Label_charge_text_size, sp2px(11));
        mTextBold = ta.getBoolean(R.styleable.Charge_Label_charge_text_bold, true);
        mTextAllCaps = ta.getBoolean(R.styleable.Charge_Label_charge_text_all_caps, true);
        mFillTriangle = ta.getBoolean(R.styleable.Charge_Label_charge_fill_triangle, true);
        mBackgroundColor = ta.getColor(R.styleable.Charge_Label_charge_background_color, Color.parseColor("#FF4081"));
        mMinSize = ta.getDimension(R.styleable.Charge_Label_charge_min_size, mFillTriangle ? dp2px(35) : dp2px(50));
        mPcharge_ding = ta.getDimension(R.styleable.Charge_Label_charge_padding, dp2px(3.5f));
        mGravity = ta.getInt(R.styleable.Charge_Label_charge_gravity, Gravity.TOP | Gravity.LEFT);
        ta.recycle();
    }
    ...
    public void setPcharge_ding(float pcharge_ding) {
        mPcharge_ding = dp2px(pcharge_ding);
        invalidate();
    }
    ...
    public float getPcharge_ding() {
        return mPcharge_ding;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mFillTriangle) {
            ...
            double delta = (textHeight + mPcharge_ding * 2) * Math.sqrt(2);
            if (mGravity == (Gravity.TOP | Gravity.LEFT)) {
                mPath.reset();
    ...
    private void drawText(int size, float degrees, Canvas canvas, float textHeight, boolean isTop) {
        ...
        float delta = isTop ? -(textHeight + mPcharge_ding * 2) / 2 : (textHeight + mPcharge_ding * 2) / 2;
    ...
    private int measureWidth(int widthMeasureSpec) {
        ...
        if (specMode == MeasureSpec.EXACTLY) {//大小确定直接使用
            ...
            int pcharge_ding = getPaddingLeft() + getPaddingRight();
            mTextPaint.setColor(mTextColor);
            mTextPaint.setTextSize(mTextSize);
            float textWidth = mTextPaint.measureText(mTextContent + "");
            result = (int) ((pcharge_ding + (int) textWidth) * Math.sqrt(2));
